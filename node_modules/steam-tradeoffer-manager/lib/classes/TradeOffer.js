...
var ETradeStatus = TradeOfferManager.ETradeStatus;
...
TradeOffer.prototype.getExchangeDetails = function(getDetailsIfFailed, callback) {
	...
	var offer = this;
	var hadEscrow = offer.escrowEnds && offer.escrowEnds.getTime() > 0;

	// 15 days is normal; 2 days if you move your authenticator to a new device; 1 day if you've been friends for a year
	var validEscrowDays = [15, 2, 1];
	makeRequest(validEscrowDays[0]);

	function makeRequest(tryEscrowDays) {
		var timestamp = Math.floor(offer.updated.getTime() / 1000);

		if (hadEscrow && offer.state != ETradeOfferState.InEscrow) {
			// If it's in escrow, then the update time will be the time of the trade's creation anyway. If not, we need math.
			timestamp = Math.floor(offer.escrowEnds.getTime() / 1000) - (60 * 60 * 24 * tryEscrowDays);
		}

		offer.manager._apiCall("GET", "GetTradeHistory", 1, {"max_trades": 100, "include_failed": 1, "start_after_time": timestamp + 60}, (err, result) => {
			if (err) {
				Helpers.makeAnError(err, callback);
				return;
			}
			...
			var trade = null;
			for (var i = 0; i < result.response.trades.length; i++) {
				if (result.response.trades[i].tradeid == offer.tradeID) {
					trade = result.response.trades[i];
					break;
				}
			}

			var durationIndex = validEscrowDays.indexOf(tryEscrowDays);

			if (!trade && hadEscrow && offer.state != ETradeOfferState.InEscrow && durationIndex != -1 && durationIndex != validEscrowDays.length - 1) {
				// Maybe this trade was held for less time?
				makeRequest(validEscrowDays[durationIndex + 1]);
				return;
			} else if (!trade) {
				Helpers.makeAnError(new Error("Trade not found in GetTradeHistory response; try again later"), callback);
				return;
			}
